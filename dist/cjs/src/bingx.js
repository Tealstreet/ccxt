...
class bingx extends Exchange["default"] {
    ...
    async setMarginMode(marginMode, symbol = undefined, params = {}) {
        ...
        if ((marginMode !== 'ISOLATED') && (marginMode !== 'CROSSED')) {
            throw new errors.ArgumentsRequired(this.id + ' (' + marginMode + ') ' + ' setMarginMode() marginMode must be "isolated" or "crossed"');
        }
        ...
        try {
            ...
            if (e instanceof errors.ExchangeError) {
                if (e.toString().indexOf('80001') >= 0) {
                    throw new errors.ExchangeError(this.id + ' ' + this.json({ 'code': 80001, 'msg': 'Cannot switch Margin Type for market with open positions or orders.' }));
                }
            }
            throw e;
        }
    ...
    async createOrder(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        if (type === 'stop') {
            if (isTakeProfitOrder) {
                ...
                throw new errors.ArgumentsRequired('unknown order direction for TP/SL');
            }
        }
        if (type === 'stopLimit') {
