...
// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
class blofin extends Exchange["default"] {
    describe() {
    ...
    parseBalance(response) {
        ...
        for (let i = 0; i < balances.length; i++) {
            ...
            account['free'] = Precise["default"].stringAdd(this.safeString(balance, 'available'), this.safeString(balance, 'bonus'));
            result[code] = account;
        }
    ...
    parsePosition(position, market = undefined) {
        ...
        if (market['inverse']) {
            const notionalValue = Precise["default"].stringDiv(Precise["default"].stringMul(contractsString, market['contractSize']), entryPriceString);
            maintenanceMarginPercentage = Precise["default"].stringDiv(maintenanceMarginString, notionalValue);
            initialMarginPercentage = this.parseNumber(Precise["default"].stringDiv(initialMarginString, notionalValue, 4));
        }
        else {
            maintenanceMarginPercentage = Precise["default"].stringDiv(maintenanceMarginString, notionalString);
            initialMarginPercentage = this.parseNumber(Precise["default"].stringDiv(initialMarginString, notionalString, 4));
        }
        const rounder = '0.00005'; // round to closest 0.01%
        maintenanceMarginPercentage = this.parseNumber(Precise["default"].stringDiv(Precise["default"].stringAdd(maintenanceMarginPercentage, rounder), '1', 4));
        const collateralString = Precise["default"].stringAdd(initialMarginString, unrealizedPnlString);
        ...
        if (side === 'net') {
            side = Precise["default"].stringGt(contractsString, '0') ? 'long' : 'short';
        }
        if (side === 'short' && Precise["default"].stringGt(contractsString, '0')) {
